					...
					void **ctxPtr)
{
	...
	for (uint64 batch_start_block = aes_start_block; batch_start_block < aes_end_block; batch_start_block += NUM_AES_BLOCKS_IN_BATCH)
	{
		...
		/*
		 * XOR the data with the AES-CTR key stream. The key stream is tied to
		 * the absolute file position, so byte N must always use the same key
		 * stream byte regardless of the caller's start_offset. enc_key is
		 * block-aligned, so the first batch starts reading it at aes_block_no
		 * (= start_offset % AES_BLOCK_SIZE); later batches start at 0.
		 *
		 * Done word-at-a-time so the compiler can vectorize it; the old
		 * per-byte loop was a bottleneck on the hot WAL write path. memcpy()
		 * handles the possibly unaligned load/store and compiles to plain
		 * word moves.
		 */
		{
			uint32		key_offset = (batch_no > 0 ? 0 : aes_block_no);
			const char *in = data + data_index;
			const unsigned char *ks = enc_key + key_offset;
			char	   *o = out + data_index;
			uint32		i = 0;

			for (; i + sizeof(uint64) <= current_batch_bytes; i += sizeof(uint64))
			{
				uint64		dv;
				uint64		kv;

				memcpy(&dv, in + i, sizeof(uint64));
				memcpy(&kv, ks + i, sizeof(uint64));
				dv ^= kv;
				memcpy(o + i, &dv, sizeof(uint64));
			}
			for (; i < current_batch_bytes; ++i)
				o[i] = in[i] ^ ks[i];
		}
