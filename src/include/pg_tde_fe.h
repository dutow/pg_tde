 ...
 * ----------------------------------------
 *
 * errmsg/errdetail/errhint are used as arguments to the redefined ereport()
 * below, which places them in a comma expression. They must therefore be
 * expressions, not statements. Return int (always 0) from the inline
 * helpers so callers can use them as comma-expression operands on every
 * compiler (including MSVC, which does not support GCC statement expressions).
 */
...
static inline int
tde_fe_errmsg(const char *fmt,...) pg_attribute_printf(1, 2);
...
static inline int
tde_fe_errdetail(const char *fmt,...) pg_attribute_printf(1, 2);
...
static inline int
tde_fe_errhint(const char *fmt,...) pg_attribute_printf(1, 2);
